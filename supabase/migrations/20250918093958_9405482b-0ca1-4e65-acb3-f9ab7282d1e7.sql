...
UPDATE product_variants SET stock_quantity = 1 WHERE id IN (
  SELECT id FROM product_variants WHERE stock_quantity NOT IN (5, 15, 3) ORDER BY RANDOM() LIMIT 2
);

UPDATE product_variants SET stock_quantity = 50 WHERE stock_quantity NOT IN (5, 15, 3, 1);
...
-- Créer des commandes fictives pour le dashboard avec des statuts valides
...
-- Commande 1
((SELECT user_id FROM profiles WHERE email = 'iryna.but@epitech.digital' LIMIT 1), 'EDN-2024-001-1234', 'delivered', 129.99, 'paid', '{"street": "15 rue de la Paix", "city": "Paris", "postal_code": "75001", "country": "France"}', '{"street": "15 rue de la Paix", "city": "Paris", "postal_code": "75001", "country": "France"}', NOW() - INTERVAL '15 days'),
...
WHERE o.order_number LIKE 'EDN-2024-%' AND NOT EXISTS (
